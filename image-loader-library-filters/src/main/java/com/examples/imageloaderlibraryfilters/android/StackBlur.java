 ...
 * <p>
 * Formatted by Mariusz Gołębiowski :-)
 */
public class StackBlur implements BitmapTransformation {
    ...
    private final short[] STACK_BLUR_MUL = {
    ...
    private final byte[] STACK_BLUR_SHR = {
    ...
    public Bitmap transform(Bitmap toTransform) {
        ...
        toTransform.getPixels(currentPixels, 0, w, 0, 0, w, h);
    ...
    private class BlurTask implements Callable<Void> {
        ...
        private void blurIteration(int[] src, int w, int h, int radius, int cores, int core, int step) {
            ...
            int hm = h - 1;
            int div = (radius * 2) + 1;
            int mul_sum = STACK_BLUR_MUL[radius];
            byte shr_sum = STACK_BLUR_SHR[radius];
